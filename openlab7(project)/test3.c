...
void printStudentall(STUDENT key);
void saveStudent(STUDENT key);





void preorderPrint(FILE* out, TreeNode* root) {
	if (root != NULL) {
		printStudentall(root->key);
		fprintf(out, "%s %s %s %d\n", root->key.id, root->key.name, root->key.course, root->key.score);
